...
import jkugiya.moneytransfer.api
...
/**
  * Implementation of the MoneytransferService.
  */
class MoneytransferServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends MoneytransferService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[MoneytransferCommand] =
    clusterSharding.entityRefFor(MoneytransferState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(MoneytransferEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[MoneytransferEvent]

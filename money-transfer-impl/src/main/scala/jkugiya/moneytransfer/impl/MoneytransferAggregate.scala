...
/**
  * This provides an event sourced behavior. It has a state, [[MoneytransferState]], which
  ...
  */
object MoneytransferBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[MoneytransferCommand]): Behavior[MoneytransferCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, MoneytransferEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[MoneytransferCommand, MoneytransferEvent, MoneytransferState](
        persistenceId = persistenceId,
        emptyState = MoneytransferState.initial,
...
/**
  ...
  */
case class MoneytransferState(message: String, timestamp: String) {
  def applyCommand(cmd: MoneytransferCommand): ReplyEffect[MoneytransferEvent, MoneytransferState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: MoneytransferEvent): MoneytransferState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[MoneytransferEvent, MoneytransferState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[MoneytransferEvent, MoneytransferState] =
    Effect
...
object MoneytransferState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: MoneytransferState = MoneytransferState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[MoneytransferCommand]("MoneytransferAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[MoneytransferState] = Json.format
}

/**
  * This interface defines all the events that the MoneytransferAggregate supports.
  */
sealed trait MoneytransferEvent extends AggregateEvent[MoneytransferEvent] {
  def aggregateTag: AggregateEventTag[MoneytransferEvent] = MoneytransferEvent.Tag
}

object MoneytransferEvent {
  val Tag: AggregateEventTag[MoneytransferEvent] = AggregateEventTag[MoneytransferEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MoneytransferEvent
...
/**
  ...
  */
trait MoneytransferCommandSerializable

/**
  * This interface defines all the commands that the MoneytransferAggregate supports.
  */
sealed trait MoneytransferCommand
    extends MoneytransferCommandSerializable
...
object MoneytransferSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    ...
    JsonSerializer[MoneytransferState],
